...
pub trait ITokenFactory<TContractState> {
    ...
    fn create_token_at(ref self: TContractState, owner: ContractAddress) -> ContractAddress;
...
pub mod TokenFactory {
    ...
    impl Factory of super::ITokenFactory<ContractState> {
        fn create_token_at(ref self: ContractState, owner: ContractAddress) -> ContractAddress {
            // Contructor arguments

            let mut constructor_calldata: Array<felt252> = array![];
            (self.token_class_hash.read(), owner).serialize(ref constructor_calldata);
            // Contract deployment
